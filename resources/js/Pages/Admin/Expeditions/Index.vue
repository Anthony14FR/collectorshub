      ...
      <div class="flex-1 flex flex-col lg:flex-row gap-4 px-2 md:px-4 lg:px-8 min-h-0 pb-4">
        <div class="flex-1 lg:mr-4 flex flex-col min-h-0" style="max-height: 85vh;">
          ...
          <div class="bg-base-100/60 backdrop-blur-sm rounded-b-xl border border-base-300/30 overflow-hidden flex flex-col flex-1">
                  ...
                  <tr v-for="expedition in sortedExpeditions" :key="expedition.id" 
                    ...
                    <td class="px-2 md:px-4 py-4">
                      <div class="min-w-0 flex-1">
                        ...
                        <div class="text-xs text-base-content/60 truncate max-w-[200px]">{{ expedition.description }}</div>
                      </div>
                    </td>
                    ...
                    <td class="px-2 md:px-4 py-4 text-center">
                      <span :class="[
                        ...
                        <div class="w-1.5 h-1.5 rounded-full" :class="expedition.is_active ? 'bg-success' : 'bg-error'"></div>
                    ...
                    <td class="px-2 md:px-4 py-4 text-center">
                      <div class="relative group">
                        ...
                        <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 w-80 bg-gray-800 text-white rounded-lg shadow-lg p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-[10000]">
                          <div v-if="expedition.rewards?.length" class="space-y-2">
                            <div v-for="reward in expedition.rewards" :key="reward.type" class="flex items-center justify-between gap-2 text-sm">
                              <div class="flex items-center gap-2">
                                <div class="w-6 h-6 flex items-center justify-center">
                                  <span v-if="reward.type === 'cash'" class="text-green-400">💰</span>
                                  <span v-else-if="reward.type === 'xp'" class="text-blue-400">⭐</span>
                                  <img v-else-if="reward.type === 'pokeball'" src="/images/items/pokeball.png" alt="Pokéball" class="w-5 h-5">
                                  <img v-else-if="reward.type === 'masterball'" src="/images/items/masterball.png" alt="Masterball" class="w-5 h-5">
                                  <span v-else class="text-purple-400">🎁</span>
                                </div>
                                <span class="text-gray-200">{{ reward.type === 'cash' ? 'Cash' : reward.type === 'xp' ? 'XP' : reward.type === 'pokeball' ? 'Pokéball' : reward.type === 'masterball' ? 'Masterball' : 'Item' }}</span>
                              </div>
                      ...
                      <div class="relative group">
                        ...
                        <div class="absolute top-full left-1/2 transform -translate-x-1/2 mt-2 w-80 bg-gray-800 text-white rounded-lg shadow-lg p-4 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none z-[10000]">
                          <div v-if="expedition.requirements?.length" class="space-y-1.5">
                            <div v-for="req in expedition.requirements" :key="req.type + req.value" class="flex items-center justify-between gap-2 text-xs">
                              <div class="flex items-center gap-2">
                                <div class="w-6 h-6 flex items-center justify-center">
                                  <div v-if="req.type === 'rarity'" class="w-2 h-2 rounded-full" :class="getRarityDotColor(req.value)"></div>
                                  <img v-else-if="req.type === 'type'" :src="`/images/types/${req.value}.png`" :alt="req.value" class="w-5 h-5 object-contain">
                                  <span v-else class="text-blue-400">📝</span>
                                </div>
                                <span class="text-gray-200">{{ req.type === 'rarity' ? 'Rareté' : 'Type' }} {{ req.value }}</span>
                              </div>
                    ...
                    <td class="px-2 md:px-4 py-4">
                      <div class="flex justify-center gap-1 flex-wrap">
                        ...
                        <Button
                          @click="toggleExpedition(expedition.id)"
                          :variant="expedition.is_active ? 'outline' : 'secondary'"
                          size="sm"
                        ...
                        <Button
                          @click="duplicateExpedition(expedition.id)"
                          variant="outline"
                          size="sm"
                        ...
                        <Button
                          @click="deleteExpedition(expedition.id)"
                          variant="outline"
                          size="sm"
            ...
            <div v-if="expeditions.links && expeditions.links.length > 3" class="flex justify-center p-4 border-t border-base-300/30">
              <div class="join">
            ...
            <div class="bg-base-100/60 backdrop-blur-sm rounded-xl border border-base-300/30 overflow-hidden">
              ...
              <div class="p-3 space-y-3">
                ...
                <div class="border-t border-base-300/30 pt-3">
                  ...
                  <div class="space-y-1">
                    <div v-for="(count, rarity) in stats.by_rarity" :key="rarity" class="flex justify-between items-center text-xs">
                      <div class="flex items-center gap-2">
