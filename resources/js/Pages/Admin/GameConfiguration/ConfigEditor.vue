<template>
  <div class="space-y-6">
    <div v-if="config.category === 'level_rewards'" class="space-y-4">
      ...
      <div v-for="(reward, key) in value" :key="key" class="bg-base-200/30 rounded-lg p-4 border border-base-300/20">
        <div class="flex items-center gap-2 mb-3">
          <span class="text-lg">{{ getMilestoneIcon(key) }}</span>
          <h4 class="font-bold text-base-content">{{ getMilestoneLabel(key) }}</h4>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          <div>
            ...
            <input
              v-model.number="value[key].pokeballs"
              type="number"
              min="0"
              class="input input-bordered input-sm w-full"
            />
          ...
          <div>
            ...
            <input
              v-model.number="value[key].masterballs"
              type="number"
              min="0"
              class="input input-bordered input-sm w-full"
            />
    ...
    <div v-else-if="config.category === 'rarity_probabilities'" class="space-y-4">
      ...
      <div v-for="(ball, ballType) in value" :key="ballType" class="bg-base-200/30 rounded-lg p-4 border border-base-300/20">
        <div class="flex items-center gap-2 mb-3">
          <span class="text-lg">{{ getBallIcon(ballType) }}</span>
          <h4 class="font-bold text-base-content">{{ ballType }}</h4>
        </div>
        
        <div class="grid grid-cols-2 gap-4">
          <div v-for="(prob, rarity) in ball" :key="rarity" class="flex items-center gap-3">
            <div class="flex items-center gap-2 min-w-20">
              <span class="w-3 h-3 rounded-full" :class="getRarityColor(rarity)"></span>
              <span class="text-sm font-medium">{{ getRarityLabel(rarity) }}</span>
            </div>
            <div class="flex-1">
              <input
                v-model.number="value[ballType][rarity]"
                type="number"
                step="0.1"
                min="0"
                max="100"
    ...
    <div v-else-if="config.category === 'xp_rewards'" class="space-y-4">
      ...
      <div v-for="(rewards, rarity) in value" :key="rarity" class="bg-base-200/30 rounded-lg p-4 border border-base-300/20">
        <div class="flex items-center gap-2 mb-3">
          <span class="w-4 h-4 rounded-full" :class="getRarityColor(rarity)"></span>
          <h4 class="font-bold text-base-content">{{ getRarityLabel(rarity) }}</h4>
        </div>
        ...
        <div class="grid grid-cols-3 gap-4">
          <div>
            ...
            <input
              v-model.number="value[rarity].base"
              type="number"
              min="0"
              class="input input-bordered input-sm w-full"
            />
          ...
          <div>
            ...
            <input
              v-model.number="value[rarity].shiny"
              type="number"
              min="0"
              class="input input-bordered input-sm w-full"
            />
          ...
          <div>
            ...
            <input
              v-model.number="value[rarity].first_catch_bonus"
              type="number"
              min="0"
              class="input input-bordered input-sm w-full"
            />
